...
export class NoteSyncExtension {
    ...
    private channel: vscode.OutputChannel = vscode.window.createOutputChannel('Note Sync')
    ...
    //判断插件是否开启
    getEnabled(): boolean {
        return !!this.context.globalState.get('enabled', true) && !!this.config.enableNoteSync
    }
    //右下角弹框提示
    ...
    private showStatusMessage(message: string) {
        let disposable = vscode.window.setStatusBarMessage(message, this.config.get('timeout') || 1000)
    ...
    pushCode() {
        let path = vscode.workspace.rootPath
        let pushShell = this.pullCommand() ??
            `git -C ${path} add .&&git -C ${path} commit -m ${this.config.pushCommit ?? "note sync plugin synchronization"}&&git -C ${path} push -u origin HEAD`
